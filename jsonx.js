        ...
        helper = {
            ...
            attributes: function (ele, map) {
                ...
                if (map === undefined) {
                    ...
                    for (name in map) {
                        helper.attr(ele, notXml, name, map[name]);
                    }
                }
            },
            attrFix: {
            ...
            merge: function (first, second) {
                var i = first.length, j = 0, k;
                if (helper.type(second.length) === 'number') {
                    for (k = second.length; j < k; j++) {
                        first[i++] = second[j];
                    }
                } else {
            ...
            propHooks: {},
            support: (function () {
                var a, div = document.createElement('div'), input, opt, select;
                ...
                if (helper.isArray(obj)) {
                    ...
                    if (obj.length > 1) {
                        if (helper.isPlainObject(obj[1])) {
                            ...
                            if (obj.length > 2) {
                                array = Array.prototype.slice.call(obj, 2);
                            }
                        } else {
                            array = Array.prototype.slice.call(obj, 1);
                        }
                        if (array.length) {
                            convertJsonx(array, ele);
                        }
                    }
    ...
    if (helper.type(JSONX.parse) !== 'function') {
        ...
        JSONX.parse = function (value) {
            ...
            function convertNodes(array) {
                var attrs, contents, i = 0, j, obj, ret = [];
